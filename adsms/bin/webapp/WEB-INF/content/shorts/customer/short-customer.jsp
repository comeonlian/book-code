...
<div class="currloca">
  ...
  <div class="sitemap">
   ...
   <form id="mainForm" action="short-customer!list.action" method="post">
  ...
  <div id="operate" class="operate">
    <div><img src="${ctx}/images/b_add.gif" alt="增加" onclick="opr_input('short-customer!input.action',false,'${authId}');"/></div>
    <div><img src="${ctx}/images/b_edit.gif" alt="修改" onclick="opr_update('short-customer!input.action','mainForm',false,'${authId}');" /></div>
    <div><img src="${ctx}/images/b_del.gif" alt="删除" onclick="opr_delete('short-customer!delAll.action','mainForm');" /></div>
  </div>
    <!-- start--------------------------------------- -->
      <table class="mtab" cellpadding="2" cellspacing="1" border="0">
			...
			<c:forEach items="${page.result}" var="a" varStatus="c">
				<tr align="center">
					...
					<td>${a.apkdays}</td>
					<td>
						<c:if test="${a.provider == 0}"><span style="color:red;">不指定</span></c:if>

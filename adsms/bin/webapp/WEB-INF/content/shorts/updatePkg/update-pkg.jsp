...
<div class="currloca">
  ...
  <div class="sitemap">
   ...
   <form id="mainForm" action="update-pkg!list.action" method="post">
  ...
  <div id="operate" class="operate">
    <div><img src="${ctx}/images/b_add.gif" alt="增加" onclick="opr_input('update-pkg!input.action',false,'${authId}');"/></div>
    <div><img src="${ctx}/images/b_edit.gif" alt="修改" onclick="opr_update('update-pkg!input.action','mainForm',false,'${authId}');" /></div>
    <!-- <div><img src="${ctx}/images/b_del.gif" alt="删除" onclick="opr_delete('update-pkg!delAll.action','mainForm');" /></div> -->
  </div>
    <!-- start--------------------------------------- -->
      <table class="mtab" cellpadding="2" cellspacing="1" border="0">
			...
			<c:forEach items="${page.result}" var="a" varStatus="c">
				<tr align="center">
					...
					<td>${a.packagename}</td>
					<td>${a.apkVersion}</td>
					<td><a href="update-pkg!detail.action?id=${a.id}&authId=${authId}"><b>
						<c:if test="${fn:length(a.customers) <= 50}">
							${a.customers}
						</c:if>
						<c:if test="${fn:length(a.customers) > 50}">
							${fn:substring(a.customers, 0, 48)}...
						</c:if>
					</b></a></td>
					<td>

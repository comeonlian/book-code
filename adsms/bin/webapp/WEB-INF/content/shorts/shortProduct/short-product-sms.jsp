...
<script type="text/javascript">
	$(function() {
		...
		$("#smsType").val("${param.filter_EQI_smsType}");
...
<div class="currloca">
  ...
  <div class="sitemap">
   ...
   <form id="mainForm" action="short-product-sms!list.action" method="post">
   ...
   <div id="filter" class="filter">
    ...
    <div class="where">
        ...
        <div class="and">
           <span>通道类型:</span><select name="filter_EQI_smsType" class="small" id="smsType">
  ...
  <div id="operate" class="operate">
    <div><img src="${ctx}/images/b_add.gif" alt="增加" onclick="opr_input('short-product-sms!input.action',false,'${authId}');"/></div>
    <div><img src="${ctx}/images/b_edit.gif" alt="修改" onclick="opr_update('short-product-sms!input.action','mainForm',false,'${authId}');" /></div>
    <!--  <div><img src="${ctx}/images/b_del.gif" alt="删除" onclick="opr_delete('short-product-sms!delAll.action','mainForm');" /></div>-->
  </div>
    <!-- start--------------------------------------- -->
      <table class="mtab" cellpadding="2" cellspacing="1" border="0">
			...
			<c:forEach items="${page.result}" var="a" varStatus="c">
				<tr align="center">
					...
					<td>
						<a href="../shortProduct/short-product-active!list.action?authId=${authId}&pid=${a.id}"><b>激活设置</b></a>
					</td>
					<td>${a.serviceCode}</td>
					<td>${a.serviceName}</td>
					<td>${a.price}</td>
					<td>
					    <c:if test="${a.smsType == 1}">SMS</c:if>
						<c:if test="${a.smsType == 2}">WAP</c:if>
						<c:if test="${a.smsType == 3}"><span style="color:red;">IVR</span></c:if>
					</td>
					<td>${a.netMode}</td>
					<td>${a.priority}</td>
					<td>

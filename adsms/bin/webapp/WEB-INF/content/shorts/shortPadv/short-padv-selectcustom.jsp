...
<body>
   ...
   <form id="mainForm" action="push-apk!selectcustom.action" method="post">
   ...
   <div id="filter" class="filter">
    ...
    <div class="where">
    	...
    	<div class="and">
           <span>客户名称:</span><input type="text" class="small" name="filter_LIKES_name" value="${param['filter_LIKES_name']}" />
        </div>
    </div>
      <div class="space"></div>
    ...
    <!-- start--------------------------------------- -->
      <table class="mtab tab_sel" cellpadding="2" cellspacing="1" border="0">
			...
			<c:forEach items="${pageCustomer.result}" var="a" varStatus="c">
				<tr align="center">
					<td><input type="radio"  value="${a.customid}" id="id_${a.customid}" name="ids"/></td>
					<td><a href="#" onclick="checkid('${a.customid}');">${a.customid}</a></td>
					<td>${a.name}</td>
					<td>${a.begintime}</td>
					<td>${a.endtime}</td>
					<td>${a.smem}</td>
	  ...
	  <wlps:page page="${pageCustomer}" showPageSize="true" excelExp="false" /></div>
    <!-- end--------------------------------------- -->
</div>

...
<script type="text/javascript">
	$(document).ready(
			function() {
				...
				$("#filter_GED_creatTime").attr("value",
						"${param['filter_GED_inTime']}");
				$("#filter_LED_inTime").attr("value",
						"${param['filter_LED_inTime']}");
	...
	$(document).ready(function() {
	    ...
	    $("#inTimeS").datepicker($.datepicker.regional['zh-CN']);
	    $("#inTimeE").datepicker($.datepicker.regional['zh-CN']);
	});
</script>
</head>
<body>
...
<div class="container"><!-- 内容区域 -->
	...
	<form id="mainForm" action="integration-soap-log.action" method="post">
	    ...
	    <div id="item" class="item">
	    	...
	    	<span style=" width:80px;">结束日志时间:</span>
	    	   ...
	    	   <div class="and">
	    		...
	    		<input type="text" id="inTimeS" name="filter_GED_inTime"  value="${param['filter_GED_inTime']}" 
	    		pattern="yyyy-MM-dd HH:mm:ss SSS"/>
	    	  </div>
	    	   <div class="and">
	    	     <span style=" width:80px;">结束创建时间:</span>
	    		<input type="text" id="inTimeE" name="filter_LED_inTime"  value="${param['filter_LED_inTime']}" 
	    		pattern="yyyy-MM-dd HH:mm:ss SSS"/>
	    	  </div>
		     </div>
	      <div class="space"></div>
...
<table class="tab_cont" width="100%" cellspacing="0" cellpadding="0"
	...
	<tbody>
		<tr>
			...
			<th><a href="javascript:sort('inTime','asc')">In创建时间</a></th>
			<th><a href="javascript:sort('outTime','asc')">Out创建时间</a></th>
			<th><a href="javascript:sort('address','asc')">URI</a></th>
		</tr>
		...
		<c:forEach items="${page.result}" varStatus="c" var="s">
			<tr>
				...
				<td >
				<a href="${ctx}/common/integration-soap-log!detail.action?id=${s.id }&tradeSeq=${s.tradeSeq}&authId=${authId}">${s.tradeSeq}</a></td>
				<td>${s.businessCode}</td>
				<td><fmt:formatDate value="${s.tradeTime}" pattern="yyyy-MM-dd HH:mm:ss SSS" /></td>
				<td><fmt:formatDate value="${s.inTime}" pattern="yyyy-MM-dd HH:mm:ss SSS" /></td>
				<td><fmt:formatDate value="${s.outTime}" pattern="yyyy-MM-dd HH:mm:ss SSS" /></td>
				<td>${s.address}</td>
			</tr>
		</c:forEach>

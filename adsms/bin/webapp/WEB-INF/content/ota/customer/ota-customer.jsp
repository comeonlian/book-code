...
<div class="currloca">
  ...
  <div class="sitemap">
   ...
   <form id="mainForm" action="ota-customer!list.action" method="post">
   ...
   <div id="filter" class="filter">
    ...
    <div class="where">
        <div class="and">
           <span>客户ID:</span>
           <input type="text" class="small" name="filter_LIKES_customerid" value="${param['filter_LIKES_customerid']}" />
        </div>
        <div class="and">
           <span>名称:</span><input type="text" class="small" name="filter_LIKES_customername" value="${param['filter_LIKES_customername']}" />
        </div>
        <div class="and">
  ...
  <div id="operate" class="operate">
    <div><img src="${ctx}/images/b_add.gif" alt="增加" onclick="opr_input('ota-customer!input.action',false,'${authId}');"/></div>
    <div><img src="${ctx}/images/b_edit.gif" alt="修改" onclick="opr_update('ota-customer!input.action','mainForm',false,'${authId}');" /></div>
    <div><img src="${ctx}/images/b_del.gif" alt="删除" onclick="opr_delete('ota-customer!delAll.action','mainForm');" /></div>
  </div>
    <!-- start--------------------------------------- -->
      <table class="mtab" cellpadding="2" cellspacing="1" border="0">
			...
			<c:forEach items="${page.result}" var="a" varStatus="c">
				<tr align="center">
					...
					<td>${a.begintime}</td>
					<td>${a.endtime}</td>
					<td>${a.linkinterval}</td>
					<td>${a.apkdays}</td>
					<td>
						<c:if test="${a.ptype == 0}"><span style="color:#FF6600;">不指定</span></c:if>
						<c:if test="${a.ptype == 1}">IVR</c:if>
						<c:if test="${a.ptype == 2}">SMS</c:if>
					</td>
					<td>
						<c:if test="${a.provider == 0}"><span style="color:#FF6600;">不指定</span></c:if>

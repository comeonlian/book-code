...
<div class="currloca">
  ...
  <div class="sitemap">
   ...
   <form id="mainForm" action="short-log!visitImsi.action" method="post">
    <input type="hidden" name="pageImsi.pageNo" id="pageNo" value="${pageImsi.pageNo}"/>
    <input type="hidden" name="pageImsi.orderBy" id="orderBy" value="${pageImsi.orderBy}"/>
    <input type="hidden" name="pageImsi.order" id="order" value="${pageImsi.order}"/>
    <input type="hidden" name="authId" id="authId" value="${authId}"/>
    <input type="hidden" name="pageImsi.pageSize" id="pageSize" value="${pageImsi.pageSize}" />
    <input type="hidden" name="pageImsi.excelExp" id="excelExp" value="${pageImsi.excelExp}" />
   <!-- 查询条件 -->
  ...
  <div id="operate" class="operate">
    ...
    <div id="indiv" style="width:100%;OVERFLOW-X:auto;">
      <table class="mtab" cellpadding="2" cellspacing="1" border="0">
			...
			<c:forEach items="${pageImsi.result}" var="a" varStatus="c">
				<tr align="center">
					<td>${a.customerid}</td>
					...
					<td>${a.ip}</td>
					<td>
						<c:if test="${a.providerCode == 0}">未知</c:if>
						<c:if test="${a.providerCode == 1}">移动</c:if>
						<c:if test="${a.providerCode == 2}">联通</c:if>
						<c:if test="${a.providerCode == 3}">电信</c:if>
					</td>
					<td>
						<c:if test="${a.data == true}">是</c:if>
						<c:if test="${a.data == false}">否</c:if>
					</td>
				</tr>
			</c:forEach>
		</table>
	  ...
	  <wlps:page page="${pageImsi}" showPageSize="true" excelExp="false" /></div>
    <!-- end--------------------------------------- -->

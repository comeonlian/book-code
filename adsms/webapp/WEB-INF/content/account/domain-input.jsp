      ...
      <div id="content" class="content input">
        <div id="indiv" style="width:100%;OVERFLOW-X:auto;">
            ...
            <input name="base" value="true" type="hidden" />
                ...
                <input name="autoCode" value="1010000000000000000" type="hidden" />
                <table class="tab_cont" width="" cellspacing="0" cellpadding="0" border="0" align="left">
                    <tbody>
                        ...
                        <tr>
                            <td  class="right"><span class="red">*</span>归属省份: </td>
                            <td><wlps:dicSelect dcode="monitor_province_info" name="location" icode="${location}"></wlps:dicSelect> </td>
                        </tr>                        
                        <tr>
                            <td  class="right">上级栏目: <img class="tip" src="../images/s_info.gif" alt="" align="absmiddle"/></td>
                            <td>
                            <select id="pId" class="text middle" name="pId" tip="选择所属级别">
                                <!-- 剔除非domain类型的集成子类 -->
                                <c:forEach items="${cats}" var="c">
                                <c:if test="${c.class == 'class com.game.entity.account.Domain'}">
                                <option value="${c.id}" <c:if test="${c.id==parent.id}">selected="selected"</c:if>>${c.formatLabel}</option>
                                </c:if>
                                </c:forEach>
                            </select>
                            <input type="hidden" name="oldPId" value="${parent.id}"/><!-- support autoCode build! by javen -->
                            </td>
                        </tr>
                        <!--
                        <tr>

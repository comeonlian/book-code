...
<%@ include file="/common/head.jsp" %>
	...
	<script type="text/javascript" src="${ctx}/js/MonthPicker/SimpleCanleder.js"></script>
	<link rel='STYLESHEET' type='text/css' href='${ctx}/js/MonthPicker/SimpleCanleder.css' />

<script type="text/javascript">
$(function(){
	$("#beginDate").css("width", "300px");
	$("#dbMonth").css("width", "300px");
	$("#dateType").val("${param.dateType}");
	$("#inCountry").val("${param.inCountry}");
	
	$("#beginmonth").simpleCanleder();
	$("#endmonth").simpleCanleder();
...
<div class="currloca">
  ...
  <div class="sitemap">
   ...
   <form id="mainForm" action="provider-daily!partnerView.action" method="post">
   ...
   <div id="filter" class="filter">
    ...
    <div class="where"> 
    	<div class="and">
           <span>查询方式:</span><select name="dateType" class="small" id="dateType">
           		<option value="1" selected="selected">按日</option>
           		<option value="2">按月</option>
           </select>
        </div>
    	<div class="and">
           <span>展示国家:</span><select name="inCountry" class="small" id="inCountry">
        ...
        <div class="and" id="beginDate">
           <span>开始日期从:</span><input type="text" style="width:90px;" class="small" id="begindate" name="begindate" value="${begindate}" />
           &nbsp;~&nbsp;<input type="text" style="width:90px;" class="small" id="enddate" name="enddate" value="${enddate}" />
        </div>
        <div class="and" style="display:none;" id="dbMonth">
           <span>开始月份从:</span><input type="text" style="width:90px;" class="small" id="beginmonth" name="beginmonth" value="${beginmonth}" />
           &nbsp;~&nbsp;<input type="text" style="width:90px;" class="small" id="endmonth" name="endmonth" value="${endmonth}" />
        </div>
    </div>
      <div class="space"></div>
  ...
  <div id="operate" class="operate">
    ...
    <!-- start--------------------------------------- -->
      <table class="mtab" cellpadding="2" cellspacing="1" border="0">
			<tr align="center">
				<th>日期</th>
				<c:if test="${inCountry == 2}"><th>国家</th></c:if>
				<th>客户ID</th>
				<th>用户数</th>
			</tr>
			<c:forEach items="${userPage.result}" var="a" varStatus="c">
				<tr align="center">
					<td>${a.currentdate}</td>
					<c:if test="${inCountry == 2}"><td>${a.countryname}</td></c:if>
					<td>${a.customid}</td>
					<td>${a.opennum}</td>
				</tr>
			</c:forEach>

...
<div class="currloca">
  ...
  <div class="sitemap">
   ...
   <form id="mainForm" action="provider-daily!list.action" method="post">
  ...
  <div id="operate" class="operate">
    <%-- <div><img src="${ctx}/images/b_add.gif" alt="增加" onclick="opr_input('provider-daily!input.action',false,'${authId}');"/></div> --%>
    <div><img src="${ctx}/images/b_edit.gif" alt="修改" onclick="opr_update('provider-daily!input.action','mainForm',false,'${authId}');" /></div>
    <%-- <div><img src="${ctx}/images/b_del.gif" alt="删除" onclick="opr_delete('provider-daily!delAll.action','mainForm');" /></div> --%>
  </div>
    <!-- start--------------------------------------- -->
      <table class="mtab" cellpadding="2" cellspacing="1" border="0">
			...
			<c:forEach items="${page.result}" var="a" varStatus="c">
				<tr align="center">
					...
					<td>
                        <a href="provider-daily!input.action?id=${a.id}&authId=${authId}">

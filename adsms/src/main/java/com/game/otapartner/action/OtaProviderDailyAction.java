package com.game.otapartner.action;
 ...
 * 合作客户数据管理Action.
 */
@Namespace("/ota/providerDaily")
@Results({ @Result(name = CrudActionSupport.RELOAD, location = "ota-provider-daily.action?authId=${authId}&oldcustomid=${oldcustomid}&cflag=${cflag}", type = "redirect") })
public class OtaProviderDailyAction extends CrudActionSupport<OtaProviderDaily> {
    ...
    @Autowired
    private OtaProviderDailyManager otaProviderDailyManager;
    ...
    private OtaProviderDaily entity;
    private Page<OtaProviderDaily> page = new Page<OtaProviderDaily>(15);
    ...
    @Override
    public OtaProviderDaily getModel() {
        return entity;
    }
    ...
    protected void prepareModel() throws Exception {
        if (null != id) {
            entity = otaProviderDailyManager.get(id);
        } else {
            entity = new OtaProviderDaily();
        }
    }
    ...
    public String list() throws Exception {
        ...
        page = otaProviderDailyManager.searchPage(page, filters);
        // excel导出
        if (page.isExcelExp())
            return toExcel(page);
    ...
    public String save() throws Exception {
        ...
        otaProviderDailyManager.save(entity);
        addActionMessage("保存 客户：" + entity.getCustomerId() + "  时间：" + entity.getCurrentdate() + " 成功");
        if (flag) {
            this.logToDB(106, "新增OtaProviderDaily-ID：" + entity.getId());
        } else {
            this.logToDB(106, "修改OtaProviderDaily-ID：" + entity.getId());
        }
        return RELOAD;
    }
    ...
    public String delAll() throws Exception {
        try {
            ...
            if (otaProviderDailyManager.delAll(ids)) {
...
//        }
        ...
        if (page.getTotalCount() > 0) {
            setProfitTotal(otaProviderDailyManager.getOpenPrice(begindate, enddate, user_customerId, dominCode));
            setActiveTotal(otaProviderDailyManager.getOpenActivity(begindate, enddate, user_customerId, dominCode));
        }
        // excel导出
        if (page.isExcelExp())
            return toExcel(page);
    ...
    public String statRetention() throws ParseException {
        ...
        userPage = otaProviderDailyManager.getRetentions(userPage, begindate,
    ...
    public String statActivitys() throws ParseException {
        ...
        userPage = otaProviderDailyManager.getActivitiusers(userPage, begindate, enddate, customerid);
        return "statActivitys";
    }

    public String statTotalUser() {

        userTotal = otaProviderDailyManager.getTotalByCusid(customerid);
        userPage = otaProviderDailyManager.getOverviewUser(userPage, customerid);
    ...
    public OtaProviderDaily getEntity() {
        return entity;
    }

    public void setEntity(OtaProviderDaily entity) {
        this.entity = entity;
    }
    ...
    public Page<OtaProviderDaily> getPage() {
        return page;
    }
    ...
    public void setPage(Page<OtaProviderDaily> page) {
        this.page = page;
    }

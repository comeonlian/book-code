...
import com.game.otapartner.service.OtaProviderDailyManager;
 ...
 * 
 */
@Service("otaProviderDailyManager")
public class OtaProviderDailyManagerImpl extends GenericManagerImpl<OtaProviderDaily, Long> implements OtaProviderDailyManager {

    private JdbcTemplate jdbcTemplate;
    private GenericDao<OtaProviderDaily, Long> otaProviderDailyDao;
    private static Logger logger = LoggerFactory.getLogger(OtaProviderDailyManagerImpl.class);
     ...
     */
    @Autowired
    public OtaProviderDailyManagerImpl(SessionFactory sessionFactory) {
        this.otaProviderDailyDao = new GenericDaoHibernate<OtaProviderDaily, Long>(OtaProviderDaily.class, sessionFactory);
        jdbcTemplate = new JdbcTemplate(SessionFactoryUtils.getDataSource(sessionFactory));
        this.dao = this.otaProviderDailyDao;
    }
    ...
    public Page<Map<String, Object>> getRetentions(Page<Map<String, Object>> page, String begindate, String enddate, String customerid) {
        ...
        sb.append(" SELECT hh.* FROM                                                                                                                           ")
                ...
                .append("     FROM                                                                                                                             ")
                .append("         (SELECT customid,DATE(accesstime) accessdate,count(deviceid) realnum FROM ota_co_device WHERE 1=1 ").append(conditionStr)
                .append(" GROUP BY customid,DATE(accesstime)) addc                 ")
                .append("     LEFT JOIN                                                                                                                        ")
                .append("         (SELECT customid,DATE(accesstime) accessdate,count(deviceid) tworet FROM ota_co_device WHERE nextretention=1 ").append(conditionStr)
                .append("  GROUP BY customid,DATE(accesstime)) twoc      ")
                .append("         ON addc.customid=twoc.customid AND addc.accessdate=twoc.accessdate                                                            ")
                .append("     LEFT JOIN                                                                                                                         ")
                .append("         (SELECT customid,DATE(accesstime) accessdate,count(deviceid) threeret FROM ota_co_device WHERE thirdretention=1 ").append(conditionStr)
                ...
                .append("     LEFT JOIN                                                                                                                         ")
                .append("         (SELECT customid,DATE(accesstime) accessdate,count(deviceid) sevenret FROM ota_co_device WHERE sevenretention=1 ").append(conditionStr)
    ...
    public Page<Map<String, Object>> getOverviewUser(Page<Map<String, Object>> page, String customerid) {
        ...
        sb.append(" SELECT customid,count(*) total from ota_co_device where 1=1 ").append(conditionStr).append(" GROUP BY customid  ORDER BY customid ");
        page = findPageOnSql(page, sb.toString());
        return page;
    }
    ...
    public Integer getTotalByCusid(String customerid) {
        ...
        sb.append("SELECT count(*) total from ota_co_device where 1=1 ").append(conditionStr);
        Integer result = jdbcTemplate.queryForInt(sb.toString());
        return result;
    }
    ...
    public Page<Map<String, Object>> getActivitiusers(Page<Map<String, Object>> page, String begindate, String enddate, String customerid) {
        ...
        sb.append(" SELECT currentdate,customid,count(1) total FROM ota_co_first_visit_daily WHERE 1=1 ").append(conditionStr).append(" GROUP BY currentdate,customid  ")
    ...
    public Integer getOpenPrice(String begindate, String enddate, String userCustomerId, String dominCode) {
        ...
        sb.append(" SELECT sum(opennum * unitprice) activitys from ota_co_provider_daily WHERE status=1 ");
        if (StringUtils.isNotBlank(begindate)) {
        ...
        int result = jdbcTemplate.queryForInt(sb.toString(), list.toArray());
        return result;
    }
    ...
    public Integer getOpenActivity(String begindate, String enddate, String userCustomerId, String dominCode) {
        StringBuilder sb = new StringBuilder();
        List<Object> list = new ArrayList<Object>();
        sb.append(" SELECT sum(opennum) activitys from ota_co_provider_daily WHERE status=1 ");
        if (StringUtils.isNotBlank(begindate)) {
            sb.append(" and currentdate>=? ");
            list.add(begindate);
        }

...
<script type="text/javascript">
	$(function() {
		$("#state").val("${param.filter_EQI_state}");
	});
...
<div class="currloca">
  ...
  <div class="sitemap">
   ...
   <form id="mainForm" action="tbl-apk!list.action" method="post">
   ...
   <div id="filter" class="filter">
    ...
    <div class="where">
    	<div class="and">
           <span>应用名:</span><input type="text" class="small" name="filter_LIKES_apkName" value="${param['filter_LIKES_apkName']}" />
        </div>
        <div class="and">
           <span>状态:</span><select name="filter_EQI_state" class="small" id="state">
  ...
  <div id="operate" class="operate">
    <div><img src="${ctx}/images/b_add.gif" alt="增加" onclick="opr_input('tbl-apk!input.action?type=${type}',false,'${authId}');"/></div>
    <div><img src="${ctx}/images/b_edit.gif" alt="修改" onclick="opr_update('tbl-apk!input.action?type=${type}','mainForm',false,'${authId}');" /></div>
    <!-- <div><img src="${ctx}/images/b_del.gif" alt="删除" onclick="opr_delete('apk!delAll.action?type=${type}','mainForm');" /></div> -->
  </div>
    <!-- start--------------------------------------- -->
      <table class="mtab" cellpadding="2" cellspacing="1" border="0">
			...
			<c:forEach items="${page.result}" var="a" varStatus="c">
				<tr align="center">
					<td><input type="checkbox" value="${a.apkId}" id="id_${a.apkId}" name="ids"/></td>
					<td>${a.apkId}</td>
					<td><a href="tbl-apk!detail.action?id=${a.apkId}&authId=${authId}"><b>${a.apkName}</b></a></td>
					<td>${a.apkPackage}</td>
					<td>${a.apkVersion}</td>
					<td>${a.apkSize}</td>
					<td>${a.apkMd5}</td>
					...
					<td>${a.keyword}</td>
					<td>
						<c:if test="${a.state == 0}"><span style="color:red;">关闭</span></c:if>
						<c:if test="${a.state == 1}">开放</c:if>
					</td>

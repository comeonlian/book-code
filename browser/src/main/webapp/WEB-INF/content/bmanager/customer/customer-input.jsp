		    ...
		    $("#inputForm").validate({
		    	rules: {
		    		customerid: {
		    			required: true,
		    			checkrno: true,
		    			remote: "customer!checkCustomid.action?oldId=${id}"
		    		},
		    		begintime: {
		    			required: true
		    		},
			...
			<table class="tab_cont" width="" cellspacing="0" cellpadding="0" border="0" align="left">
				<tbody>
					...
					<tr>
						...
						<td class="right"><span class="red">*</span>SDK是否运行:</td>
						<td>
							<select name="isrun" id="isrun" style="width: 120px">
								<option value="0" <c:if test="${isrun == 0}">selected</c:if>>不运行</option>
								<option value="1" <c:if test="${isrun == 1}">selected</c:if>>运行</option>
							</select>
						</td>
					</tr>
